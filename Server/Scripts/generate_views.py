...
def generate_view_sql(table_name, columns):
    ...
    lines.append(f"CREATE OR ALTER VIEW [Lbys].[VEM_{table_name}] AS")
    lines.append("SELECT")
    ...
    for col_name, data_type in columns:
        ...
        prefix = "    " if first_col else "    ,"

        # Add REFERANS_TABLO_ADI after first column (usually the primary key)
        if first_col:
            col_lines.append(f"{prefix}t.[{col_name}] AS {col_name}")
        ...
        if col_name in ('EKLEYEN_KULLANICI_KODU', 'GUNCELLEYEN_KULLANICI_KODU'):
            col_lines.append(f"{prefix}COALESCE(NULLIF(t.[{col_name}], '00000000-0000-0000-0000-000000000000'), '{DEFAULT_USER_GUID}') AS {col_name}")
        else:
            col_lines.append(f"{prefix}t.[{col_name}] AS {col_name}")
...
def main():
    ...
    sql_parts.append("-- Schema: [Lbys] (VEM_ prefix views)")
    sql_parts.append("-- Includes REFERANS_TABLO_ADI column")
    sql_parts.append("-- =============================================================================")
    sql_parts.append("")
    ...
    # Write to file
    output_file = r"C:\Proje\GitHub\lbys\Server\Scripts\CreateLbysViews.sql"
