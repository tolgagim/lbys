"""
VEM 2.0 C# Entity Generator
141 tablo icin Entity siniflarini olusturur
"""

import json
import os
import re

def load_tables():
    with open('vem2_data/VEM_2.0_all_tables.json', 'r', encoding='utf-8') as f:
        return json.load(f)

def convert_type(vem_type):
    """VEM veri tipini C# tipine donustur"""
    type_map = {
        'AlfaNumerik': 'string',
        'Alfanumerik': 'string',
        'alfanumerik': 'string',
        'TarihSaat': 'DateTime?',
        'Tarihsaat': 'DateTime?',
        'tarihsaat': 'DateTime?',
        'Tarih': 'DateTime?',
        'tarih': 'DateTime?',
        'SayisalTam': 'int?',
        'Sayisaltam': 'int?',
        'sayisaltam': 'int?',
        'SayısalTam': 'int?',
        'SayisalOndalik': 'decimal?',
        'Sayisalondalik': 'decimal?',
        'SayısalOndalık': 'decimal?',
        'Mantiksal': 'bool?',
        'Mantıksal': 'bool?',
        'mantiksal': 'bool?',
    }
    return type_map.get(vem_type, 'string')

def clean_name(name):
    """VEM_ prefix'ini kaldir"""
    if name.startswith('VEM_'):
        return name[4:]
    return name

def to_pascal_case(name):
    """SNAKE_CASE'i PascalCase'e cevir (navigation property icin)"""
    parts = name.lower().split('_')
    return ''.join(word.capitalize() for word in parts)

def generate_entity(table_name, table_data, all_tables):
    """Tek bir entity sinifi olustur"""
    clean_table_name = clean_name(table_name)
    columns = table_data.get('columns', [])

    # Class baslangici
    lines = []
    lines.append(f'/// <summary>')
    lines.append(f'/// {table_name} tablosu')
    lines.append(f'/// </summary>')
    lines.append(f'public class {clean_table_name}')
    lines.append('{')

    # Foreign key referanslarini topla
    fk_references = {}

    for col in columns:
        col_name = col.get('name', '')
        col_type = col.get('type', 'AlfaNumerik')
        description = col.get('description', '')
        nullable = col.get('nullable', 'Evet')
        reference = col.get('reference', '-')

        if not col_name:
            continue

        # C# tipini belirle
        csharp_type = convert_type(col_type)

        # Nullable kontrolu
        if nullable == 'Hayir' or nullable == 'Hayır':
            if csharp_type == 'string':
                csharp_type = 'string'  # string zaten nullable
            elif csharp_type.endswith('?'):
                csharp_type = csharp_type[:-1]  # ? kaldir

        # Property yaz
        if description and description != '-':
            # Aciklamayi temizle
            desc_clean = description.replace('\n', ' ').replace('\r', ' ')
            if len(desc_clean) > 100:
                desc_clean = desc_clean[:100] + '...'
            lines.append(f'    /// <summary>{desc_clean}</summary>')

        lines.append(f'    public {csharp_type} {col_name} {{ get; set; }}')
        lines.append('')

        # Foreign key referansi varsa kaydet
        if reference and reference != '-' and reference.startswith('VEM_'):
            fk_references[col_name] = reference

    # Navigation properties ekle
    if fk_references:
        lines.append('    // Navigation Properties')
        for col_name, ref_table in fk_references.items():
            clean_ref = clean_name(ref_table)
            nav_name = to_pascal_case(clean_name(ref_table))
            lines.append(f'    public virtual {clean_ref}? {nav_name} {{ get; set; }}')
        lines.append('')

    lines.append('}')

    return '\n'.join(lines)

def generate_all_entities():
    """Tum entity siniflarini olustur"""
    tables = load_tables()

    # Entities klasoru olustur
    output_dir = 'Entities'
    os.makedirs(output_dir, exist_ok=True)

    all_entities = []

    # Her tablo icin entity olustur
    for table_name, table_data in tables.items():
        entity_code = generate_entity(table_name, table_data, tables)
        clean_name_str = clean_name(table_name)
        all_entities.append((clean_name_str, entity_code))

        # Ayri dosyaya yaz
        file_path = os.path.join(output_dir, f'{clean_name_str}.cs')
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write('namespace Lbys.Entities;\n\n')
            f.write(entity_code)

        print(f'  {clean_name_str}.cs olusturuldu')

    # Tek dosyada tum entity'leri birlestir
    combined_path = os.path.join(output_dir, '_AllEntities.cs')
    with open(combined_path, 'w', encoding='utf-8') as f:
        f.write('namespace Lbys.Entities;\n\n')
        f.write('// VEM 2.0 - 141 Tablo Entity Siniflari\n')
        f.write('// Otomatik olusturuldu\n\n')
        for name, code in sorted(all_entities):
            f.write(code)
            f.write('\n\n')

    print(f'\n{len(all_entities)} entity sinifi olusturuldu')
    print(f'Dosyalar: {output_dir}/')

    return all_entities

def generate_dbcontext(entities):
    """DbContext sinifini olustur"""
    lines = []
    lines.append('using Microsoft.EntityFrameworkCore;')
    lines.append('')
    lines.append('namespace Lbys.Data;')
    lines.append('')
    lines.append('public class VemDbContext : DbContext')
    lines.append('{')
    lines.append('    public VemDbContext(DbContextOptions<VemDbContext> options) : base(options) { }')
    lines.append('')

    # DbSet'ler
    for name, _ in sorted(entities):
        lines.append(f'    public DbSet<Entities.{name}> {name} {{ get; set; }}')

    lines.append('')
    lines.append('    protected override void OnModelCreating(ModelBuilder modelBuilder)')
    lines.append('    {')
    lines.append('        base.OnModelCreating(modelBuilder);')
    lines.append('')
    lines.append('        // Tablo adlarini VEM_ prefix ile ayarla')
    for name, _ in sorted(entities):
        lines.append(f'        modelBuilder.Entity<Entities.{name}>().ToTable("VEM_{name}");')
    lines.append('    }')
    lines.append('}')

    # Dosyaya yaz
    os.makedirs('Data', exist_ok=True)
    with open('Data/VemDbContext.cs', 'w', encoding='utf-8') as f:
        f.write('\n'.join(lines))

    print('DbContext olusturuldu: Data/VemDbContext.cs')

if __name__ == '__main__':
    print('='*60)
    print('VEM 2.0 C# Entity Generator')
    print('='*60)
    print('')

    entities = generate_all_entities()
    print('')
    generate_dbcontext(entities)
